...
\\Trennt die Verbindung eines Clienten zum Server und "ubergibt dabei dem Clienten eine Nachricht
\item boolean connectionLost(int playerID)
\\"Uberpr"uft ob die Verbindung zu einem Clienten unterbrochen worden ist
...
% besserer Name?
\begin{itemize}
\item Die Validierung einkommender Befehle von Clienten an den Server wurde fallen gelassen, es wird nur noch "uberpr"uft ob die Befehle spielregelkonform sind
\item Die Schnittstelle IClient kommt f"ur Kommunikationszwecke von Server zu Client dazu
\item Der Spielstand wird allein beim Server gespeichert
\item Aus Kompatibilit"atsgr"unden wird /PD01/ "`Spielfeld"' nur auf dem Server gespeichert
\item Ein Spiel startet nun, wenn die maximale Anzahl der Clienten auf einem Server verbunden ist, statt bei variabler Anzahl mit "`Bereit-Option"' starten zu können
\item Es gibt keine M"oglichkeit, aktiv ein Spiel aufzugeben (/F12080/)
\item Ein AIClient kann dem Server hinzugefügt werden, anstatt dass dieser als eigenes Programm gestartet wird dass sich auf den Server verbindet

...
\item GameLogic
\begin{itemize}
\item Beinhaltet den Inhalt des Pakets und ist die Kontaktstelle nach außen.
...
\item boolean addPlayer()
\\
\item boolean addMessage()
\\
\item boolean addRoll()
\\
\item boolean startGame()
\\
...
\item GameRules
\begin{itemize}
\item Beinhaltet die Regeln des Spiels.
...
\item Methoden
\begin{itemize}
\item boolean execute(int player,int[] argument)
...
\item Image getImage()
\\Das Bild des Feldes
\item boolean execute(int player)
\\Die Aktion wenn eine Spieler auf diesem Feld landet
\item boolean passTrough(int player)
...
\item boolean sendIServer(NetOjim server)

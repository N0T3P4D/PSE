...
\subsubsection{Interfaces, abstrakte Klassen}
\begin{itemize}
\item ValuationFunction (abstrakte Klasse)
...
Zur Ausführung des am besten bewerteten Spielzugs wird das Entwurfsmuster "`Befehl"' verwendet. Das Interface "`Command"' stellt das Grundgerüst für die Befehle dar. Die zu implementierende Methode \textit{execute()} dient zur eigentlichen Befehlsausführung.
\end{itemize}
\subsubsection{Klassen}
\begin{itemize}
\item AiClient
...
\begin{itemize}
\item KI-Client: Verwenden der Entwurfsmuster "`Befehl"' und "`Singleton"'
\end{itemize}
\end{document}

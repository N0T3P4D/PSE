...
\begin{description}
\item[/F2010/] Starten des Servers \\
Der Server wird mit den entsprechenden Regeln gestartet. Diese Spielregeln können in einer Datei festgehalten werden, oder als Übergabeparameter angegeben werden. Danach befindet sich der Server im Wartemodus (-> /F2020/) und wartet auf neue Spieler. Alternativ kann auch ein gespeichertes Spiel geladen werden.
\item[/F2020/] Wartemodus \\
Im Wartemodus stellt der Server ein Chatraum zur Verfügung. Jeder Spieler landet zuerst hier und kann sich auf "`bereit"' setzen. Nachdem mindestens zwei Spieler verbunden sind und alle bereit sind, startet das Spiel (-> /F2030/).
\item[/F2030/] Spielstart \\
Das Spiel wird gestartet, indem zufällig die Reihenfolge der Spieler festgelegt wird. Der erste Spieler in der Liste startet dann das Spiel. Sollte das Spiel geladen worden sein, wird dieser Schritt übersprungen.

Nachdem alle Spieler die Reihenfolge bestätigt haben, wird der erste Spielzug gestartet indem der entsprechende Spieler informiert wird, dass er dran ist (-> /F2040/).
\end{description}
\subsubsection{Spielzug}
\begin{description}
\item[/F2040/] Spielzug melden \\
Nachdem die nächste Person bekannt ist, die zieht, wird diesen Spieler bescheid gegeben und allen anderen übermittelt, dass dieser Spieler dran ist. Danach wartet er darauf, dass gewürfelt werden soll.
\item[/F2050/] Würfeln \\
Um den Betrug des Spielers vorzubeugen bekommt der Server von den Client den Befehl zu würfeln. Dieser erzeugt dann eine zufällige Zahl zwischn 2 und 12. Alle Spieler erhalten dann diese Zahl und alle Spieler, ziehen diese Spielfigur weiter. Danach kann der aktive Spieler weitere Befehle geben.
\item[/F2060/] Weitere Befehle \\
